...
RUN go build -v -modcacherw -o /output/$TARGETOS/$TARGETARCH/session-manager-grpc-plugin-server-go .
...
# Copy build
COPY --from=builder /output/$TARGETOS/$TARGETARCH/session-manager-grpc-plugin-server-go session-manager-grpc-plugin-server-go
