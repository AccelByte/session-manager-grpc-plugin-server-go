...
RUN env GOOS=$TARGETOS GOARCH=$TARGETARCH go build -modcacherw -o session-manager-grpc-plugin-server-go_$TARGETOS-$TARGETARCH
...
COPY --from=builder /build/session-manager-grpc-plugin-server-go_$TARGETOS-$TARGETARCH session-manager-grpc-plugin-server-go
...
# Entrypoint.
CMD [ "/app/session-manager-grpc-plugin-server-go" ]